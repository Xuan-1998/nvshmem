...
static int nvshmemt_libfabric_gdr_process_completion(nvshmem_transport_t transport,
    ...
    if (entry->flags & FI_REMOTE_CQ_DATA) {
        ...
        if (NVSHMEMT_LIBFABRIC_IMM_PUT_SIGNAL_SEQ == imm_header ||
            ...
            NVSHMEM_TRACE_RECEIVER_COMPLETION_REMOTE(ep.domain_index, (uint64_t)(addr ? *addr : 0), NVSHMEMT_LIBFABRIC_REMOTE_RMA);
            goto out;
            ...
            NVSHMEM_TRACE_SENDER_COMPLETION_ACK(ep.domain_index, (uint64_t)*addr, (int)imm_header);
            goto out;
        } else {
    ...
    if (entry->flags & FI_SEND) {
        NVSHMEM_TRACE_SENDER_COMPLETION(ep.domain_index, (uint64_t)entry->op_context, op->type);
        state->op_queue[domain_idx]->putToSend(op);
        ep.completed_ops++;
    } else if (entry->flags & FI_RMA) {
        NVSHMEM_TRACE_RECEIVER_COMPLETION(ep.domain_index, (uint64_t)entry->op_context, op->type);
        ...
        status = nvshmemt_libfabric_put_signal_completion(transport, ep, entry, addr);
        NVSHMEM_TRACE_RECEIVER_COMPLETION_REMOTE(ep.domain_index, addr ? (uint64_t)*addr : 0, NVSHMEMT_LIBFABRIC_REMOTE_SIGNAL);
        ...
        if (op->type == NVSHMEMT_LIBFABRIC_ACK) {
            ...
            NVSHMEM_TRACE_SENDER_COMPLETION_AMO_RESPONSE(ep.domain_index, (uint64_t)entry->op_context);
            ...
            NVSHMEM_TRACE_RECEIVER_COMPLETION_REMOTE(ep.domain_index, (uint64_t)entry->op_context, NVSHMEMT_LIBFABRIC_REMOTE_AMO);
